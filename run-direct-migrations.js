  ...
  try {
    ...
    for (const file of migrationFiles) {
      ...
      try {
        ...
        for (const statement of statements) {
          ...
          try {
            ...
            if (error) {
              ...
              // Try executing with a different method if the first one fails
              const { data: rawData, error: rawError } = await supabase.rpc('exec', { query: statement });
              if (rawError) {
                console.error(`Error with alternative execution: ${rawError.message}`);
              }
            }
          } catch (err) {
        ...
        console.log(`✅ Successfully processed migration: ${file}`);
      } catch (err) {
