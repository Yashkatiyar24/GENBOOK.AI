...
export const initializeDatabase = async () => {
  try {
    ...
    // The migrations directory is in the project root (one level up from server directory)
    const actualProjectRoot = path.dirname(projectRoot); // Go up one more level
    const migrationsDir = path.join(actualProjectRoot, 'migrations');
    const migrationPath = path.join(migrationsDir, '20230813_add_tenant_rbac.sql');
    
    // Debug logs
    console.log('Actual project root:', actualProjectRoot);
